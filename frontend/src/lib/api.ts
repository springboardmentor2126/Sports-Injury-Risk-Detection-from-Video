...
export interface AnalysisResult {
  ...
  biomechanics: {
    ...
    avg_knee_valgus_angle: number | null;
    avg_shoulder_rotation: number | null;
  };
  risk_flags: {
    ...
    knee_valgus_frames:          number;
  };
  xgboost: {
    risk_level:    string;
    risk_score:    number;
    confidence:    number;
    probabilities: { low: number; moderate: number; high: number; critical: number };
    sport_used:    string;
    model_version: string;
  };
  // AI Recommendations (generated by Gemini at analysis time)
  ai_recommendations: {
    exercise_recommendations: string[];
    mobility_suggestions: string[];
...
export interface AnalysisHistoryItem {
  ...
  frames_knee_valgus:          number;
  // XGBoost output
  xgboost_confidence:    number | null;
  xgboost_probabilities: { low: number; moderate: number; high: number; critical: number } | null;
  sport_type_used:       string | null;
  // AI Recommendations (from Gemini, saved to DB)
  ai_recommendations: {
    exercise_recommendations: string[];
    mobility_suggestions: string[];
    recovery_planning: string[];
...
// ─── Chat API (ephemeral — NOT saved to DB) ────────────────────────────────
export const chatApi = {
  sendMessage: (sessionId: string, message: string, role: string): Promise<{ response: string }> =>
    request<{ response: string }>(`/api/chat/${sessionId}`, {
      method: "POST",
      body: JSON.stringify({ message, role }),
    }),
  sendDashboardMessage: (message: string, role: string): Promise<{ response: string }> =>
    request<{ response: string }>(`/api/chat/dashboard`, {
      method: "POST",
      body: JSON.stringify({ message, role }),
    }),
};
