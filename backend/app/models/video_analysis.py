...
class VideoAnalysis(Base):
    ...
    # XGBoost-specific metrics (Milestone 3)
    avg_knee_valgus_angle = Column(Float, nullable=True)   # Average FPPA valgus angle
    avg_shoulder_rotation = Column(Float, nullable=True)   # Average shoulder rotation
    ...
    # ── Overall Risk Assessment (XGBoost output) ──────────────────────
    # XGBoost replaces the old heuristic risk calculation from Milestone 2.
    risk_level             = Column(String(20), nullable=True)   # "low" | "moderate" | "high" | "critical"
    xgboost_confidence     = Column(Float, nullable=True)        # Confidence of predicted class (0.0-1.0)
    xgboost_probabilities  = Column(Text, nullable=True)         # JSON: {"low": 0.05, "moderate": 0.15, ...}
    sport_type_used        = Column(String(50), nullable=True)   # Sport profile used for prediction
    ai_recommendations     = Column(Text, nullable=True)         # JSON: {exercise, mobility, recovery} — generated by Gemini
